...
def test_al_superar_el_limite_no_se_llama_al_modelo(client, meta_enviados, monkeypatch):
    ...
    assert len(llamadas_al_modelo) == 3
    assert meta_enviados[-1][1] == MENSAJE_LIMITE_ALCANZADO

    # Mensaje 5: sigue por encima del límite. Silencio total, ni el aviso se repite.
    cantidad_enviados_previa = len(meta_enviados)
    _post_mensaje(client, "wamid.limite.4", "mensaje 4")
    assert len(llamadas_al_modelo) == 3
    assert len(meta_enviados) == cantidad_enviados_previa
