...
contra Codex, ni una vuelta completa por WhatsApp con IA real**. Antes de dar la
...
- **anthropic** (Codex, producción) y **`openai_compat`** (desarrollo: un solo
...
- **Toda la generación de respuestas vive detrás de
  ...
  (`respuesta.py`), seleccionable con `PROVEEDOR_IA` (`fijo`/`openai_compat`/`Codex`).
...
- **Para producción, además:** `PROVEEDOR_IA=Codex` con `ANTHROPIC_API_KEY` y
...
para `openai_compat`, un cliente falso para `Codex`). Usan una base SQLite en
